...
import { ACTIONS, apply, undo, type Change } from './settings.ts'
...
function show(change: Change, write: boolean, verb: 'apply' | 'undo'): void {
  ...
  if (change.note && !change.skip) out(change.note)
}

if (command === 'guard') {
  await (await import('./guard.ts')).guardMain(rest.includes('--plugin'))
