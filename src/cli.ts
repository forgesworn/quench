...
import { apply, SETTING_ACTIONS, undo, type Change } from './settings.ts'

const [command, ...rest] = process.argv.slice(2)
const out = (line = ''): void => { process.stdout.write(`${line}\n`) }

function show(change: Change, write: boolean, verb: 'apply' | 'undo'): void {
  out(`${verb === 'apply' ? 'Apply' : 'Undo'} ${change.action}: ${change.describe}.`)
  out(`Settings file: ${change.path}`)
  if (change.skip) out(`Nothing to do: ${change.skip}.`)
  else if (!write) out(`Nothing written yet. Run \`quench ${verb} ${change.action} --yes\` to ${verb === 'apply' ? 'make' : 'reverse'} the change.`)
  else out(`Done; the previous file was backed up beside it.${verb === 'apply' ? ` Reverse it with \`quench undo ${change.action} --yes\`. It applies to Claude Code sessions started from now on.` : ''}`)
}

if (command === 'guard') {
  await (await import('./guard.ts')).guardMain()
