...
import { claudeHome, claudeSettingsPath, pluginState, quenchHome, readSettings, readState } from './settings.ts'
...
try { settings = readSettings(claudeSettingsPath()) } catch {}
...
function appliedLines(): string[] {
  ...
  let log: Array<{ at: string; event?: string; usd?: number | null }> = []
  try { log = readFileSync(join(quenchHome(), 'guard.log'), 'utf8').trim().split('\n').filter(Boolean).map((l) => JSON.parse(l)) } catch {}
...
if (args.includes('--json')) {
  ...
  const model = (id: string): string => (id === 'other' ? 'other models' : !id.startsWith('claude-') ? id : id.replace(/^claude-/, '').replace(/-\d{8}$/, '').replace(/-(\d+)-(\d+)(?=$|-)/, ' $1.$2').replace(/-(\d+)(?=$|-)/, ' $1'))
  ...
  for (const r of reports) {
    ...
    if (r.agent === 'claude') out('    Compaction: no advice. Quench\'s one test (DeepSeek, two task chains, contexts to 110K) went both ways and nothing above 200K is tested; see docs/EVIDENCE.md, Q8.')
  }
}
