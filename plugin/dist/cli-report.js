...
import { quenchHome, readState } from "./settings.js";
...
function appliedLines() {
    ...
    if (guard) {
        let held = [];
        try {
            held = readFileSync(join(quenchHome(), 'guard.log'), 'utf8').trim().split('\n').filter(Boolean).map((l) => JSON.parse(l));
        }
        catch { }
...
if (args.includes('--json')) {
    ...
    const model = (id) => (!id.startsWith('claude-') ? id : id.replace(/^claude-/, '').replace(/-(\d+)-(\d+)(?=$|-)/, ' $1.$2').replace(/-(\d+)(?=$|-)/, ' $1').replace(/-\d{8}$/, ''));
    const setting = {
    ...
    for (const r of reports) {
        ...
        for (const a of r.actions) {
            ...
            if (a.id === 'fresh-after-break') {
                ...
                if (r.agent === 'claude')
                    out(state.applied['break-guard'] ? `     Applied: the break guard has been on since ${day(state.applied['break-guard'].at)}.` : '     Do it: quench apply break-guard (holds the first prompt after such a break once, with its cost)');
            }
