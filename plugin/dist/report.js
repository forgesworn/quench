...
const time = (stamp) => { const t = stamp ? Date.parse(stamp) : NaN; return Number.isNaN(t) ? null : t; };
/**
...
export function claudeParser() {
    ...
    return {
        line(text) {
            ...
            if (e.type !== 'assistant' || !u || !id || e.message?.model === '<synthetic>')
                return;
...
export function codexParser() {
    ...
    return {
        line(text) {
            if (!text.includes('"token_count"') && !text.includes('"turn_context"') && !text.includes('"session_meta"'))
                return;
            let e;
            try {
                e = JSON.parse(text);
            ...
            if (e.type === 'session_meta')
                sub = typeof p.source === 'object' && p.source !== null && 'subagent' in p.source;
            else if (e.type === 'turn_context' && typeof p.model === 'string')
                model = p.model;
...
export const cost = (r, p) => { const x = parts(r, p); return x.read + x.write + x.out; };
/** What the same tokens would cost on another model. */
export const costOn = (r, p, model) => cost({ ...r, model }, p);
 ...
 */
export function idleRebuilds(s) {
...
export function naturalCompactions(sessions, span = 30) {
    const out = [];
    for (const s of sessions) {
        const r = s.requests;
        const ctx = (i) => r[i].ctx;
        const growth = (a, b) => { let t = 0; for (let k = a + 1; k <= b; k += 1)
            t += Math.max(0, ctx(k) - ctx(k - 1)); return t / (b - a); };
        for (let i = span + 1; i + span < r.length; i += 1) {
            if (ctx(i) < 0.6 * ctx(i - 1) && ctx(i - 1) >= 100e3)
                out.push({ before: ctx(i - 1), after: ctx(i), growth: growth(i, i + span) / Math.max(1, growth(i - 1 - span, i - 1)) });
        }
    }
    return out;
}
 ...
 */
export function simulate(s, p, compactAt, summary) {
...
export function agentReport(agent, sessions, summaryOverride) {
    ...
    const within = (x, [lo, hi]) => x >= lo && (hi === null || x < hi);
